...
// Menentukan batas (limit) dan lompatan (offset) data
if ($is_home_utama) {
    // Jika di Beranda Utama: Halaman 1 butuh 9 berita (3 untuk highlight atas + 6 untuk kotak bawah)
    if ($halaman == 1) {
        $limit = 9;
        $offset = 0;
    } else {
        $limit = 6; // Halaman 2 dst hanya butuh 6 berita
        $offset = 9 + (($halaman - 2) * 6);
    }
    // Hitung total halaman (1 halaman pertama + sisa halaman dibagi 6)
    $total_halaman = 1 + ceil(max(0, $total_data - 9) / 6);
} else {
    // Jika sedang di Kategori Tertentu (Tidak ada highlight)
    $limit = 6;
    $offset = ($halaman - 1) * $limit;
    $total_halaman = ceil($total_data / $limit);
}

// 3. Mengambil Berita Sesuai Halaman (Paginasi)
try {
    $query = "
...
// 4. Mengambil Top 5 Berita Terpopuler (Untuk Sidebar Kanan)
try {
    $stmt_populer = $pdo->query("
        SELECT judul, slug, views, created_at 
        FROM berita 
        ORDER BY views DESC 
        LIMIT 5
    ");
...
// 5. Logika Pemisahan Highlight (Hanya jika di Halaman 1 & Beranda Utama)
...
<style>
    ...
    /* CSS UNTUK PAGINASI (SLIDE HALAMAN) */
    .pagination { display: flex; justify-content: center; gap: 8px; margin-top: 50px; padding-top: 30px; border-top: 1px solid #e2e8f0; }
    ...
    .sidebar-widget { background: #ffffff; border-radius: 12px; border: 1px solid #f1f5f9; padding: 25px; margin-bottom: 30px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); position: sticky; top: 20px; }
    ...
    @media (max-width: 992px) {
        ...
        .headline-card { height: 350px !important; }
        .side-card { height: 250px !important; }
...
<div class="home-container">
    ...
    <div class="highlight-section">
        <a href="/berita/<?= htmlspecialchars($highlight_utama['slug']) ?>" class="headline-card" style="height: 450px;">
    ...
    <div class="content-wrapper">
        ...
        <aside class="right-sidebar">
            <div class="sidebar-widget">
                <h4 class="widget-title"><i class="fa-solid fa-fire-flame-curved"></i> Paling Populer</h4>
